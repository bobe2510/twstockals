...
| **close_confirm** | 約 **13:10**：近收盤確認破防守＋提早清單＋觀測評等 |
| **eod** | 約 **14:15**：收盤停損停利／出清＋台股 ETF ≥門檻請買進 |
...
Alert／階梯進度：`reports/alert_state.json` 與 `reports/latest/deploy_ladder_state.json` 以 Actions cache 盡量保留。
