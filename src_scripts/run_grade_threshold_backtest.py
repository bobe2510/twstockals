    ...
    *,
    ...
    for i in range(n):
        ...
        if should_sell and shares > 0:
            if is_tw:
                cash = shares * px * (1 - COMM_TW - TAX_TW)
            else:
                cash = shares * px * (1 - COMM_US)
            shares = 0.0
            trades += 1

        if shares == 0 and meets_threshold(g, mode):
            if is_tw:
                shares = cash / (px * (1 + COMM_TW))
            else:
                shares = cash / (px * (1 + COMM_US))
            cash = 0.0
            trades += 1
            buys += 1
            buy_grades.append(g)
    ...
    bh = closes[test_start:] / bh_start * INITIAL
    m_bh = get_metrics(pd.Series(bh), 0, years, INITIAL)
...
def _run_modes_etf(code, name, kind, is_tw, df, taiex_df, modes, eval_years):
...
def main():
    ...
    out_md = os.path.join(WORKSPACE, "reports", "latest", "grade_threshold_backtest.md")
    out_json = os.path.join(WORKSPACE, "reports", "latest", "grade_threshold_backtest.json")
